...
local slideToFallFrameCount = 1
local textureCount = 33
...
function update(player)
    ...
    slideInFrame[player + 1] = math.min(math.floor(animeCounter[player + 1] * frameRate), 32)
    fallFrame[player + 1] = math.max(math.min(math.floor(((animeCounter[player + 1] - 1.133) * frameRate) + slideToFallFrameCount), 1), 0)
