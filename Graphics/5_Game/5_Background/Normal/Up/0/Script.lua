...
local LowTaperFade = { 0, 0 }

function updateClearFade()
    for player = 0, playerCount - 1 do
        if isClear[player] then
            LowTaperFade[player + 1] = LowTaperFade[player + 1] + (2000 * deltaTime)
        else
            LowTaperFade[player + 1] = LowTaperFade[player + 1] - (2000 * deltaTime)
        end

        if LowTaperFade[player + 1] > 255 then
            LowTaperFade[player + 1] = 255
        end
        if LowTaperFade[player + 1] < 0 then
            LowTaperFade[player + 1] = 0
        end
    end
end
...
function draw()
    ...
    if playerCount == 1 and p1IsBlue == false then
      ...
      func:SetOpacity(LowTaperFade[1], "BG_Clear.png")
      func:SetOpacity(LowTaperFade[1], "Dot_Clear.png")
      func:SetOpacity(LowTaperFade[1], "Dango_Clear.png")
    ...
    elseif playerCount == 2 then
      ...
      for player = 0, 1 do
        func:SetOpacity(LowTaperFade[player + 1], "BG_Clear.png")
        func:SetOpacity(LowTaperFade[player + 1], "Dot_Clear.png")
        func:SetOpacity(LowTaperFade[player + 1], "Dango_Clear.png")
